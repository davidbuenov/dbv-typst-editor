...
* **Próximo paso**: v0.1.0 (`/build`+`/test`+`/code-simplify`+`/ship` documental) y v0.2 completo (RF-13, RF-11, catálogo de 8 plantillas) están cerrados. Una sesión autónoma posterior (2026-09-05, usuario ausente) avanzó además 5 slices de Beta: Outline (§7.8), exportación PNG de página actual (§7.12), asistente de citas con autocompletado (§7.7.4), terminal avanzado (§7.14) y modos de escritura (§7.9) — 158 tests automáticos en verde (61 Vitest + 97 Rust), pero **ninguno de esos 5 slices se ha visto todavía en la ventana real**. Sigue pendiente la decisión de publicación de `v0.1.0` (etiquetar y publicar la Release). Ver Context Snapshot y la lista "Deliberadamente fuera de esta sesión" al final de la Fase 7.
...
- [ ] **Fase 7: Beta (v0.2–v0.4) — en curso desde el 2026-09-05.** Sesión autónoma ("avanza todo lo que puedas hasta la versión Beta"), sin el usuario disponible para resolver ambigüedades: se prioriza lo ya especificado sin preguntas abiertas, en slices verificados uno a uno, y se deja explícitamente fuera lo que exige una decisión del usuario o infraestructura no disponible aquí (ver "Deliberadamente fuera de esta sesión" al final de esta fase).
  ...
  - **Bibliografía visual** (panel de exploración de entradas `.bib`, más allá de las claves que ya usa el asistente de citas). Sigue abierta la pregunta de `SPECIFICATIONS.md` §9 sobre qué crate de parseo BibTeX usar — el escaneo ligero de `bibliography.rs` (Slice 16) resuelve el caso de las claves sin tocar esa pregunta, pero un panel visual de verdad (autor, título, año por entrada) sí la necesitaría.
