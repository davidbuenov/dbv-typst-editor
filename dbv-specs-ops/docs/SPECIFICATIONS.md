...
> **Estado:** 🔒 **CONGELADO v1.1 — 2026-09-04** (baseline de especificación para el MVP). v1.1 aplica los ajustes de alcance aprobados por el usuario al autorizar `/build`: MVP reducido al bucle de valor completo, plantilla "Proyecto en blanco", apertura de proyectos existentes como flujo de primer nivel y operaciones de proyecto explícitas. Incorpora Spec Addendum, Additional Specification Clarification, TYPST CLI INTEGRATION, el research phase dedicado y el feedback de posicionamiento de producto; ver [`ARCHITECTURE.md`](./ARCHITECTURE.md) y [`TYPST_ECOSYSTEM_RESEARCH.md`](./TYPST_ECOSYSTEM_RESEARCH.md).
...
| **v0.2** | Project Archive `.dbvt` (export/import) + 4 plantillas restantes (TFM, Tesis doctoral, Informe técnico, Presentación). | Planificado |
